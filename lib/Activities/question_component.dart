  ...
  Widget build(BuildContext context) {
    ...
    return Slidable(
      ...
      child: new Card(
        ...
        ? Column(
          children: <Widget>[
            Padding(padding: EdgeInsets.all(5.0)),Center(child: CircularProgressIndicator()),
              ...
              children: <Widget>[
                ...
                RaisedButton(
                  ...
                  onPressed: snapshot.value["user"].toString() ==
                      ...
                      ? () => update(
                          snapshot.key, snapshot.value["question"].toString())
                      : null,
                ),
              ],
            )
   ...
   void loadLikeData(String key) {
     bool liked,disliked;
    databaseReference.child(key).child("likes").child(StaticState.user.email.substring(0,StaticState.user.email.indexOf("@"))).once().then((snapshot) {
      ...
      databaseReference.child(key).child("dislikes").child(StaticState.user.email.substring(0,StaticState.user.email.indexOf("@"))).once().then((snapshot) {
      if (snapshot.value != null && snapshot.value["flag"]) {
        disliked=true;
      }
      else{
